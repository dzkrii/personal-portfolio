...
function RoleRotator() {
  ...
  useEffect(() => { const timer = window.setInterval(() => setIndex((value) => (value + 1) % roles.length), 2600); return () => window.clearInterval(timer); }, [roles.length]);
  return <span className="role-rotator" aria-live="polite"><motion.span key={roles[index]} initial={{ y: "100%", opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: "-100%", opacity: 0 }}>{roles[index]}</motion.span></span>;
}
